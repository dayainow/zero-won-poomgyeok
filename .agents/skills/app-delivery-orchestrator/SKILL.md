...
Codex Agent Teams가 가능하면 `TeamCreate`, `TaskCreate`, `SendMessage`를 사용한다. 현재 런타임에 팀 도구가 없으면 같은 에이전트 정의를 기준으로 서브 에이전트 또는 병렬 위임을 사용하고, 모든 중간 산출물은 `_workspace/`에 저장한다.
...
2. 하네스 자체가 바뀌었으면 `AGENTS.md` 변경 이력에 날짜와 이유를 남긴다.
